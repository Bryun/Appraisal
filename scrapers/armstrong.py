...
class Archer(ICounty):
    def __init__(self):
        super().__init__()
        self.COUNTY: str = "Armstrong"
        self.URL: str = ""
    ...
    async def extract_files(self):
        with ZipFile(
            f"{self.DATA_DIR}1684961236_ARMSTONG CAD 2022 CERTIFIED MINERAL OPEN RECORDS.zip"
        ) as archive:
            files = [
                x
                ...
                if search(
                    re.compile(
                        r"MINERAL DATA CSV FILE|MINERAL DATA FLAT TEXT FILE|LAYOUT - TDMO19U"
                    ),
                    x,
                )
            ]
    ...
    async def read_blueprint(self):
        content = PDF(
            f"{self.DATA_DIR}ARMSTONG CAD 2022 CERTIFIED BPP OPEN RECORDS/2. LAYOUT - TDMO19U.PDF"
        ).read()
...
# run(x.save_blueprint())
data = run(
    x.read_data(
        "../downloads/Armstrong/ARMSTONG CAD 2022 CERTIFIED BPP OPEN RECORDS/3. MINERAL DATA FLAT TEXT "
        "FILE.TXT"
    )
)
