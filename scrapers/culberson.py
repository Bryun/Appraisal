...
class Culberson(ICounty):
    def __init__(self):
        super().__init__()
        self.COUNTY: str = "Culberson"
        self.URL: str = ""
    ...
    async def extract_files(self):
        with ZipFile(
            f"{self.DATA_DIR}1689089314_CULBERSON CAD 2022 CERTIFIED MINERAL OPEN RECORDS.zip"
        ) as archive:
            files = [
                x
    ...
    async def read_blueprint(self):
        content = PDF(
            f"{self.DATA_DIR}CULBERSON CAD 2022 CERTIFIED MINERAL OPEN RECORDS/2. LAYOUT - TDMO19U.PDF"
        ).read()
...
data = run(
    x.read_data(
        f"../downloads/{x.COUNTY}/CULBERSON CAD 2022 CERTIFIED MINERAL OPEN RECORDS/3. MINERAL DATA FLAT TEXT FILE.TXT"
    )
)
