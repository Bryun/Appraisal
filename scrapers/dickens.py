...
class Dickens(ICounty):
    def __init__(self):
        super().__init__()
        self.COUNTY: str = "Dickens"
        self.URL: str = ""
    ...
    async def extract_files(self):
        with ZipFile(
            f"{self.DATA_DIR}1675096804_DICKENS CAD 2022 CERTIFIED MINERAL OPEN RECORDS.zip"
        ) as archive:
            files = [
                x
    ...
    async def read_blueprint(self):
        content = PDF(
            f"{self.DATA_DIR}DICKENS CAD 2022 CERTIFIED MINERAL OPEN RECORDS/2. LAYOUT - TDMO19U.PDF"
        ).read()
...
# data = run(
#     x.read_data(
#         f"../downloads/{x.COUNTY}/DICKENS CAD 2022 CERTIFIED MINERAL OPEN RECORDS/3. MINERAL DATA FLAT TEXT FILE.TXT"
#     )
# )
