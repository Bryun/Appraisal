...
class Fisher(ICounty):
    def __init__(self):
        super().__init__()
        self.COUNTY: str = "Fisher"
        self.URL: str = ""
    ...
    async def extract_files(self):
        with ZipFile(
            f"{self.DATA_DIR}1691004636_FISHER CAD 2023 CERTIFIED MINERAL OPEN RECORDS.zip"
        ) as archive:
            files = [
                x
                ...
                if search(
                    re.compile(
                        r"MINERAL DATA CSV FILE|MINERAL DATA FLAT TEXT FILE|LAYOUT - TMDO19U"
                    ),
                    x,
                )
            ]
    ...
    async def read_blueprint(self):
        content = PDF(
            f"{self.DATA_DIR}FISHER CAD 2023 CERTIFIED MINERAL OPEN RECORDS/2. LAYOUT - TMDO19U.PDF"
        ).read()
...
# data = run(
#     x.read_data(
#         f"../downloads/{x.COUNTY}/FISHER CAD 2023 CERTIFIED MINERAL OPEN RECORDS/3. MINERAL DATA FLAT TEXT FILE.TXT"
#     )
# )
