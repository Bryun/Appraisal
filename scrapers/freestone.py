...
class Freestone(ICounty):
    def __init__(self):
        super().__init__()
        self.COUNTY: str = "Freestone"
        self.URL: str = ""
    ...
    async def extract_files(self):
        with ZipFile(
            f"{self.DATA_DIR}1691165658_FREESTONE CAD 2023 CERTIFIED MINERAL OPEN RECORDS.zip"
        ) as archive:
            files = [
                x
    ...
    async def read_blueprint(self):
        content = PDF(f"{self.DATA_DIR}2. LAYOUT - TMDO19U.PDF").read()
