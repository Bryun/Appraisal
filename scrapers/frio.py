...
class Frio(ICounty):
    ...
    async def extract_files(self):
        with ZipFile(
            f"{self.DATA_DIR}1673015214_FRIO CAD 2022 CERTIFIED MINERAL OPEN RECORDS.zip"
        ) as archive:
            files = [
                x
                for x in archive.namelist()
                if search(
                    re.compile(r"MINERAL DATA FLAT TAX FILE|LAYOUT - TDMO19U"),
                    x,
                )
            ]
    ...
    async def read_blueprint(self):
        content = PDF(
            f"{self.DATA_DIR}FRIO CAD 2022 CERTIFIED MINERAL OPEN RECORDS/2. LAYOUT - TDMO19U.PDF"
        ).read()
...
data = run(
    x.read_data(
        f"../downloads/{x.COUNTY}/FRIO CAD 2022 CERTIFIED MINERAL OPEN RECORDS/3. MINERAL DATA FLAT TAX FILE.TXT"
    )
)
