...
class Glasscock(ICounty):
    def __init__(self):
        super().__init__()
        self.COUNTY: str = "Glasscock"
        self.URL: str = ""
    ...
    async def extract_files(self):
        with ZipFile(
            f"{self.DATA_DIR}1670009085_GLASSCOCK CAD 2022 CERTIFIED MINERAL OPEN RECORDS.zip"
        ) as archive:
            files = [
                x
    ...
    async def read_blueprint(self):
        content = PDF(f"{self.DATA_DIR}2. LAYOUT - TDMO19U.PDF").read()
