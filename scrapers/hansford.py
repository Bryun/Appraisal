...
class Hansford(ICounty):
    def __init__(self):
        super().__init__()
        self.COUNTY: str = "Hansford"
        self.URL: str = ""
    ...
    async def extract_files(self):
        with ZipFile(
            f"{self.DATA_DIR}1689602289_HANSFORD CAD 2023 CERTIFIED MINERAL FILES FOR PC.zip"
        ) as archive:
            files = [
                x
                for x in archive.namelist()
                if search(
                    re.compile(r"MN309801"),
                    x,
                )
            ]
...
data = run(x.read_data(f"../downloads/{x.COUNTY}/MN309801.TXT"))
