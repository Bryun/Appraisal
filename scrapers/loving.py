...
class Loving(ICounty):
    def __init__(self):
        super().__init__()
        self.COUNTY: str = "Loving"
        self.URL: str = ""
    ...
    async def extract_files(self):
        with ZipFile(
            f"{self.DATA_DIR}1672929601_LOVING CAD 2022 CERTIFIED MINERAL OPEN RECORDS.zip"
        ) as archive:
            files = [
                x
    ...
    async def read_blueprint(self):
        content = PDF(
            f"{self.DATA_DIR}LOVING CAD 2022 CERTIFIED MINERAL OPEN RECORDS/2. LAYOUT - TDMO19U.PDF"
        ).read()
...
data = run(
    x.read_data(
        f"../downloads/{x.COUNTY}/LOVING CAD 2022 CERTIFIED MINERAL OPEN RECORDS/3. MINERAL DATA FLAT TEXT FILE.TXT"
    )
)
