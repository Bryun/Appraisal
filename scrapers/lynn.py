...
class Lynn(ICounty):
    ...
    async def extract_files(self):
        with ZipFile(
            f"{self.DATA_DIR}1685642570_LYNN CAD 2022 CERTIFIED MINERAL OPEN RECORDS.zip"
        ) as archive:
            files = [
                x
    ...
    async def read_blueprint(self):
        content = PDF(
            f"{self.DATA_DIR}LYNN CAD 2022 CERTIFIED MINERAL OPEN RECORDS/2. LAYOUT - TDMO19U.PDF"
        ).read()
...
data = run(
    x.read_data(
        f"../downloads/{x.COUNTY}/LYNN CAD 2022 CERTIFIED MINERAL OPEN RECORDS/3. MINERAL DATA FLAT TEXT FILE.TXT"
    )
)
