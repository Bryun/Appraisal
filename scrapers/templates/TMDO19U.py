...
class TMDO19U(ICounty):
    def __init__(self, county="TMDO19U"):
        super().__init__()
        ...
        if self.URL is not None:
            with Scraper(url=self.URL, download_dir=self.DATA_DIR) as browser:
                element = browser.driver.find_element(
                    By.XPATH,
                    "//h4[contains(text(), '2023 Certified Appraisal Rolls')]",
                )
    ...
    async def extract_files(
        self,
        names: list = (
            "MINERAL DATA CSV FILE",
            "MINERAL DATA FLAT TEXT FILE",
            "LAYOUT - TMDO19U",
        ),
    ):
        for x in listdir(self.DATA_DIR):
            name, extension = splitext(x)

            if extension == "zip":
                with ZipFile(f"{self.DATA_DIR}{x}") as archive:
                    files = [
                        y
                        for y in archive.namelist()
                        if search(re.compile("|".join(name)), y)
                    ]
    ...
    async def read_blueprint(self):
        files = [f for f in listdir(self.DATA_DIR) if isfile(f)]
        layout = filter(lambda f: f.endswith((".pdf", ".PDF")), files)[0]

        content = PDF(f"{self.DATA_DIR}{layout}").read()
        ...
        with SQLite(path="../appraisals.db") as db:
            for i in data.index:
                ...
                sql = (
                    ...
                    "Metadata) VALUES (?, ?, ?, ?, ?, ?, (SELECT ID FROM County WHERE Name = ?), ?);"
                )
                await db.insert(
                    sql,
                    [
    ...
    async def read_data(self):
        files = [f for f in listdir(self.DATA_DIR) if isfile(f)]
        data = filter(
            lambda f: f.startswith(("3. MINERAL DATA FLAT TEXT FILE.TXT")), files
        )[0]
        path = f"{self.DATA_DIR}{data}"
...
#
# data = run(x.read_data(f"../downloads/{x.COUNTY}/3. MINERAL DATA FLAT TEXT FILE.TXT"))
